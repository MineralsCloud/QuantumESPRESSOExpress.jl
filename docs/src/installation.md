...
## Install `QuantumESPRESSOExpress`
...
version of `QuantumESPRESSOExpress`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/QuantumESPRESSOExpress.jl"
```

in the second step above.

## Update `QuantumESPRESSOExpress`
...
our [GitHub repository](https://github.com/MineralsCloud/QuantumESPRESSOExpress.jl)
for new releases.
Once we release a new version, you can update `QuantumESPRESSOExpress` by typing

```@repl
using Pkg
Pkg.update("QuantumESPRESSOExpress")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `QuantumESPRESSOExpress`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("QuantumESPRESSOExpress")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `QuantumESPRESSOExpress`.
