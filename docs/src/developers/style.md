...
!!! info
    The style guide is always a work in progress, and not all QuantumESPRESSOExpress code
    follows the rules. When modifying QuantumESPRESSOExpress, please fix the style violations
...
`QuantumESPRESSOExpress` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/QuantumESPRESSOExpress.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `QuantumESPRESSOExpress` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `QuantumESPRESSOExpress` have
